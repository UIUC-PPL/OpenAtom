...
void LAPLACE::initialize(double* e_occ, double* e_unocc, int nocc, int nunocc){
  minEocc = e_occ[0];
  maxEocc = e_occ[nocc-1];
  minEunocc = e_unocc[0];
...
void LAPLACE::set_GapBandwidth(double* e_occ, double* e_unocc, int nocc, int nunocc){

  if( e_occ[0] < minEocc ){
    minEocc = e_occ[0];
  }
  if( e_occ[nocc-1] > maxEocc ){
    maxEocc = e_occ[nocc-1];
  }
  if( e_unocc[0] < minEunocc ){
    minEunocc = e_unocc[0];
  }
  if( e_unocc[nunocc-1] > maxEunocc ){
    maxEunocc = e_unocc[nunocc-1];
  }
...
void LAPLACE::reset_metal_gap(double* e_occ, double* e_unocc, int nocc, int nunocc, int index){
  // if metal calculations, need to define Egap again
  // also num occupied states may vary depending on k points,
  // so we reset nv and nc

  // reset maxEocc 
  double maxEocctmp;
  int nvcount = 0;
  for (int i=0; i<nocc+nunocc; i++){
    // keep updating & count occupied states
    double eig;
    if(i < nocc)
      eig = e_occ[i];
    else
  ...
  nvk[index]=nvcount;
  printf("maxEocctmp = %f maxEocc = %f \n",maxEocctmp, maxEocc);
  if ( maxEocctmp > maxEocc ){
    maxEocc = maxEocctmp;
  }
  printf("new maxEocc = %f\n",maxEocc);

  // reset minEunocc
  double minEunocctmp;
  int nccount = 0;
  for (int i=nocc+nunocc; i>0; i--){
    double eig;
    if(i-1 < nocc)
      eig = e_occ[i-1];
    else
      eig = e_unocc[i-1-nocc];
    // keep updating and count unoccupied states
    if ( eig > Ef ){
      minEunocctmp = eig;
      nccount += 1;
    }
  ...
  nck[index] = nccount;
  if ( minEunocctmp < minEunocc ){
    minEunocc = minEunocctmp;
  }
  printf("new minEunocc = %f\n",minEunocc); 
  // check if nvk+nck=nstate
  GWBSE *gwbse = GWBSE::get();
  if (nvk[index] + nck[index] == gwbse->gw_parallel.L + gwbse->gw_parallel.M){//usrin->nstate){
    printf("Reset nv and nc at this k point: nvk=%d, nck=%d\n",nvk[index],nck[index]);
  }
  else{
    CkPrintf("\nError nvk+nck is not equal to nstate");
    CkExit(1);
  }
...
void LAPLACE::set_windows(){
  ...
  GWBSE *gwbse = GWBSE::get();
  int nEv = gwbse->gw_parallel.L*gwbse->gw_parallel.K;
  int nEc = gwbse->gw_parallel.M * gwbse->gw_parallel.K;
...
//printf("\nstep_occ = %lf, step_ouncc = %lf\n", step_occ, step_unocc);

  // set-up the bin
  for (int i=1; i<nwocc; i++){
    bin_occ[i] = bin_occ[i-1] + slice_occ[i-1]*step_occ;
  }
  // for occupied states, shifted states can be read. 
  // For that case, it is necessary to increase the bin size for the edge bins. 
  bin_occ[0] -= 3e-3;
  bin_occ[nwocc] += 3e-3;
...
void LAPLACE::set_numnodes(){
  ...
  for (int i=0; i<nwocc; i++){
	...
	estimated *= 1.0/a;
	double errorG = fabs(exact - estimated) * G * 100;
	...
	estimated *= 1.0/a;
	double errorW = fabs(exact - estimated) * W * 100;
...
//printf("\n%lf, %lf, %lf\n", gap, width, npts);
  ...
  int nmax = 10000;
  double* ulist = new double[nmax];
  
  double step;
  step = 4.0/double((nmax-1));

  // make ulist
  for (int i=0; i<nmax; i++){
    double p = -2+step*i;
    ulist[i] = std::pow(10.0,p);
  }
  
  double exact = 1.0; // exact value of the integrand
  double* errdiff = new double[nmax];
  for (int i=0; i<nmax; i++){
