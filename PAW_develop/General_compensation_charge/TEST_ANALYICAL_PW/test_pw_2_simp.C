...
// check each step of the simplification 
   double temp = (derfP)*(rlt2/(rgt2*rgt))  
               + (derfM-gaussH*rlt*(1.0+mombo))*(rgt2/(rlt2*rlt))
               - (gaussH)*((4.0*beta4*combo -2.0*argE + 3.0)*sinchH 
                                            +(2.0*argE - 3.0)*coshH - 4.0*beta4*rgt4*(1.0+mombo))/(argH*argH);
//==========================================================================
   return temp;
 }// end routine
//==========================================================================
