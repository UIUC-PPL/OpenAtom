...
//	gen_Gauss_quad_driver(type, thetaorder, iopt, xtheta_master, wtheta_master, &ierr_zero1, &ierr_ortho1);
	...
	if (model == 2) {
		...
		for (int i=0; i < rorder; i++) {
			...
			psi0_2[i] = x2;  // the weight function of the quadrature has the Gaussian
			wr_master[i] *= psi0_2[i]*norm; // unitless
			check += wr_master[i];
		} // end for
		PRINTF("check = %g\n", check);
	} // end if
	...
	if (model == 1) {
		...
		for (int i=0; i < rorder; i++) {
			...
			psi0_2[i] = sin(ak1*r)*sin(ak1*r); // ak1*r is unitless
			wr_master[i] = delta_master*psi0_2[i]*2.0/Rpc_master; // unitless
			check += wr_master[i];
		} // end for
		wr_master[0] *= 0.5;
		wr_master[rorder-1] *= 0.5;
		PRINTF("check = %g\n", check);
	} // end if
