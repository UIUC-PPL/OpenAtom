...
\newcommand{\rgt}{r_{>}}
\newcommand{\rlt}{r_{<}}
\newcommand{\brgt}{\bar{r}_{>}}
\newcommand{\brlt}{\bar{r}_{<}}
\newcommand{\Derfp}{\mathrm{D}^{(\mathrm{erf})}_p}
\newcommand{\Derfm}{\mathrm{D}^{(\mathrm{erf})}_m}
\newcommand{\tDerfmS}{\tilde{\mathrm{D}}^{(\mathrm{erf,2})}_m}
\newcommand{\tDerfmF}{\tilde{\mathrm{D}}^{(\mathrm{erf,4})}_m}
\newcommand{\Derfcp}{\mathrm{D}^{(\mathrm{erfc})}_p}
\newcommand{\Derfcm}{\mathrm{D}^{(\mathrm{erfc})}_m}
\newcommand{\erfcp}{\mathrm{E}^{(\mathrm{erfc})}_p}
\newcommand{\erfcm}{\mathrm{E}^{(\mathrm{erfc})}_m}
...
The partial wave expansion of the regularized / screened Coulomb interaction can
be expressed as
\begin{equation}
\label{eq:modBerfPw}
\begin{split}
\frac{\rerf(\beta|\br-\br'|)}{|\br-\br'|} &= \left(\frac{2}{\sqrt{\pi}}\right)\frac{1}{|\br-\br'|}
\int_0^{\beta|\br-\br'|} \rd u\ \re^{-u^2} \\
&=  \frac{2}{\sqrt{\pi}}\int_0^\beta \rd u\ \re^{-u^2(r^2 + r'^2)}\re^{2u^2rr'\cos(\theta)} \\
...
\PWerf_l(r,r';\beta) = \left(\frac{2(2l+1)}{\sqrt{\pi}}\right)\left(\frac{1}{\sqrt{2rr'}}\right)
      \int_0^{\beta\sqrt{2rr'}}\rd x\ \re^{-x^2\left(\frac{r^2 + r'^2}{2rr'}\right)}i_{l}(x^2),
\end{equation}
the $i_l$ are the modified spherical Bessel function of the 1st kind, here, given up to f-wave
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
and the expansion of the exponential of the cosine of the angle between the vectors $\br$ and $\br'$, $\cos(\theta)$,
in terms of the $i_l$ and the Legendre polynomials is taken from formula 10.2.36 of Abramowitz and Stegun.
...
The factor $4\pi/(2l+1)$ is introduced such that in the first key limit,
$\lim_{\beta\rightarrow\infty}\PWerf_l(r,r';\beta)=\rlt^l/\rgt^{l+1}$, the familiar partial wave expansion of
$1/|\br-\br'|$ emerges, where $\rgt = \mathrm{max}(r,r')$ and $\rlt = \mathrm{min}(r,r')$.  The second key limit,
$\lim_{r'\rightarrow 0}\PWerf_l(r,r';\beta)=(\rerf(\beta r)/r)\delta_{l,0}=(\rerf(\beta \rgt)/\rgt)\delta_{l,0}$
while obvious from Eq. (\ref{eq:modBerfPw}) can proved in the integral form as follows
\begin{eqnarray}
\lim_{r'\rightarrow 0} \PWerf_l(r,r';\beta) &=& \lim_{r'\rightarrow 0}\left(\frac{2(2l+1)}{\sqrt{\pi}}\right)
\int_0^{\beta}\rd x\ \re^{-x^2\left(r^2 + r'^2\right)}i_{l}(2rr'x^2) \nonumber\\
&=& \lim_{r'\rightarrow 0}\left(\frac{2(2l+1)}{\sqrt{\pi}}\right)
\int_0^{\beta}\rd x\ \re^{-x^2 r^2}i_{l}(0) \nonumber \\
&=& \lim_{r'\rightarrow 0}\left(\frac{2(2l+1)}{\sqrt{\pi}}\right)
\int_0^{\beta}\rd x\ \re^{-x^2r^2}\delta_{l,0} \nonumber \\
&=& \left(\frac{2(2l+1)}{\sqrt{\pi}}\right)\int_0^{\beta}\rd x\ \re^{-x^2r^2}\delta_{l,0} \nonumber \\
&=& (2l+1) \frac{\erf(\beta r)}{r}\delta_{l,0} = \frac{\erf(\beta \rgt)}{\rgt}\delta_{l,0}
\end{eqnarray}
The result, $\lim_{y\rightarrow 0}i_{l}(y)=y^l/(2l+1)!!=\delta_{l,0}$, given in formula (10.2.5) of
Abramowitz and Stegun and the definition of $\{\rgt,\rlt\}$ have been applied.

In order to proceed, the required indefinite integrals are evaluated in closed form
\begin{eqnarray}
%-----------------------------------------------
\int \rd x\ \re^{-a^2x^2}i_{0}(x^2)&=& 
\frac{\sqrt{\pi}\sgn(x)}{2}\left [a_m \rerfc(a_m |x|) - a_p \rerfc(a_p |x|) \right ] 
- xe^{-a^2x^2} \left [\sinhc(x^2)\right] \hspace*{4in}
\end{eqnarray}

\begin{eqnarray}
%-----------------------------------------------
\int \rd x\ \re^{-a^2x^2}i_{1}(x^2)&=& 
   \frac{\sqrt{\pi}\mathrm{sgn}(x)}{6}\left [ (2 a^2 + 1)a_m \rerfc(a_m |x|)  - (2 a^2 - 1)a_p \rerfc(a_p|x|) 
   \right ] \hspace*{4in} \nonumber \\
...
\frac{\sqrt{\pi} \mathrm{sgn}(x)}{10}\left [
 ...
 -(4 a^4 - 2 a^2 - 1) a_p \rerfc(a_p|x|) \right ] \hspace*{4in} \nonumber \\
 &-& \frac{e^{-a^2 x^2} }{5x^3}\left [
 ( (4 a^4-1)x^4 - 2 a^2 x^2 + 3)\sinhc(x^2) + (2 a^2 x^2 - 3)\cosh(x^2)
  \right ] 
\end{eqnarray}

\begin{eqnarray}
\int \rd x\ \re^{-a^2x^2}i_{3}(x^2) &=&
\frac{\sqrt{\pi}\mathrm{sgn}(x)}{14}\left [
  (8 a^6 + 4 a^3 - 4a^2 - 1) a_m \rerfc(a_m|x|) 
 -(8 a^6 - 4 a^2 - 4a^2 + 1) a_p \rerfc(a_p|x|) \right ] \hspace*{4in} \nonumber \\
 &-& \frac{e^{-a^2 x^2} }{7x^5}\left [
      ((-8a^4 + 4)a^2x^6  + (+4a^4 + 4)x^4 - 6a^2x^2 + 15 )\sinhc(x^2) \right .\nonumber \\
& & \left . \hspace*{1.5in}
     +((-4a^4 + 1)x^4  + 6a^2x^2 - 15)\cosh(x^2) 
  \right ] 
\end{eqnarray}
where
\begin{equation}
...
\end{split}
\end{equation}
The integrals are presented such that they are relatively easy to check by differentiation wrt to x.\\

In order to place the partial waves of Eq. (\ref{eq:ScrPw}) in simplified form, it is first helpful to recognize
that $\lim \beta\rightarrow \infty$ can only be realized if the lower limit of the integrals above
when combined with the prefactor yields $\rlt^l/\rgt^{l+1}$.
Second, since the upper and lower limits of the integrals are positive semi-definite ($x \geq 0$),
the absolute value of $x$ in the closed form expressions above is not necessary and we can set 
simply set $\sgn(x)=1$ and $|x|=x$.
The necessity of defining $\rgt= \mathrm{max}(r,r')$ and $\rlt= \mathrm{min}(r,r')$ arises from
the definition a's in terms of the r's
\begin{equation}
\begin{split}
a   &=  \frac{\sqrt{r^2+{r'}^2}}{\sqrt{2rr'}} = \frac{\sqrt{\rgt^2+\rlt^2}}{\sqrt{2\rgt\rlt}} \\
a_m &=  \frac{|r - r'|}{\sqrt{2rr'}}            = \frac{\rgt - \rlt}{\sqrt{2\rgt\rlt}} \\
a_p &=  \frac{r + r'}{\sqrt{2rr'}}             = \frac{\rgt + \rlt}{\sqrt{2\rgt\rlt}} 
\end{split}
\end{equation}
Below, the partial waves are written in terms $\{\rgt,\rlt\}$ in the standard way, as opposed 
to retaining the $\{r,r'\}$ form which requires explicitly referencing max and min functions.

Using the simplified form of the integrals above and the definition of the a's in terms of $\rgt$ and $\rlt$,
the coefficients of the partial wave expansion of the regularized / cutoff / screened Coulomb interaction can be
written in
a form in which the limits $\rlt\rightarrow 0$, and $\beta\rightarrow \infty$
can be evaluated by inspection,
\begin{eqnarray}
\PWerf_0(r,r';\beta) &=& 
\Derfp(\rlt,\rgt;\beta)\left[\frac{1}{\rgt}\right] 
+\left[\Derfm(\rlt,\rgt;\beta)-\tDerfmS(\rlt,\rgt;\beta)\right]\left[\frac{1}{\rlt}\right] \hspace*{4in}\\
&-& G_H(\rlt,\rgt,\beta)
   \left [\sinhc(2\brgt\brlt)-1\right ] \nonumber
\end{eqnarray}
%
\begin{eqnarray}
  \PWerf_1(r,r';\beta) &=&
   \Derfp(\rlt,\rgt;\beta) \left [\frac{\rlt}{\rgt^2} \right] 
+  \left [\Derfm(\rlt,\rgt;\beta)-\tDerfmS(\rlt,\rgt;\beta)\right]
   \left [\frac{\rgt}{\rlt^2} \right] \hspace*{6.in}\\
&-&  G_H(\rlt,\rgt,\beta)
    \left [(2(\brgt^2+\brlt^2)-1)\sinhc(2\brgt\brlt) 
          + \cosh(2\brgt\brlt)- 2\brgt^2\right]\left[\frac{1}{2\brgt\brlt}\right ]
\nonumber 
\end{eqnarray}
%
\begin{eqnarray}
\PWerf_2(r,r';\beta) &=&   
    \Derfp(\rlt,\rgt;\beta) \left [\frac{\rlt^2}{\rgt^3} \right] 
+  \left[\Derfm(\rlt,\rgt;\beta) -\tDerfmF(\rlt,\rgt;\beta) \right]
   \left [\frac{\rgt^2}{\rlt^3} \right] \hspace*{6.in}\nonumber \\
&-&  G_H(\rlt,\rgt,\beta)
    \Bigl [(4(\brgt^4+\brlt^4+\brgt^2\brlt^2)-2(\brgt^2+\brlt^2) + 3)\sinhc(2\brgt\brlt)
\Bigr . \nonumber \\ 
& &       \hspace*{2.1in}  + (2(\brgt^2+\brlt^2) - 3)\cosh(2\brgt\brlt)  \nonumber \\
& &\Bigl .\hspace*{0.8in}  - 4\brgt^4 \left(1+\frac{2\brlt^2(\brgt^2+1)}{3}\right) \Bigr ]
   \left [\frac{1}{4\brgt^2\brlt^2}\right ] \nonumber
\end{eqnarray}

\begin{eqnarray}
\PWerf_3(r,r';\beta) &=&    \Derfp(\rlt,\rgt;\beta) \left [\frac{\rlt^3}{\rgt^4} \right] 
+  \left[\Derfm(\rlt,\rgt;\beta) -\tDerfmF(\rlt,\rgt;\beta) \right]
   \left [\frac{\rgt^3}{\rlt^4} \right] \hspace*{6.in}\nonumber \\
&-&  G_H(\rlt,\rgt,\beta)
    \Bigl [(-15 + 6(\brgt^2+\brlt^2) - 4(\brgt^4 + \brlt^4 + 6\brgt^2\brlt^2) \Bigr . \nonumber \\
& &     \hspace*{1.6in}  
           + 8(\brgt^6 + \brlt^4\brgt^2 + \brlt^2\brgt^4 + \brlt^6))\sinhc(2\brgt\brlt) \nonumber \\ 
& &    \hspace*{0.9in}  
       + (15 - 6(\brgt^2+\brlt^2) + 4(\brgt^4 + \brlt^4 + \brlt^2\brgt^2))\cosh(2\brgt\brlt)  \nonumber \\
& &\Bigl .\hspace*{0.9in}  - 8\brgt^6\left(1 + \frac{2\brlt^2(\brgt^2+1)}{3} \right) \Bigr ]
   \left [\frac{1}{8\brgt^3\brlt^3}\right ] 
\end{eqnarray}
Here, for convenience, the functions,
\begin{equation}
\begin{split}
  \rgt &= \mathrm{max}(r,r') \\
  \rlt &= \mathrm{min}(r,r') \\
  \brgt &= \beta\rgt \\
  \brlt &= \beta\rlt \\
  \Derfp(\rlt,\rgt;\beta) &=\frac{1}{2}\left[\rerf(\brgt+\brlt)+\rerf(\brgt-\brlt)\right]\\
  \Derfm(\rlt,\rgt;\beta) &=\frac{1}{2}\left[\rerf(\brgt+\brlt)-\rerf(\brgt-\brlt)\right] \\
  G_H(\rlt,\rgt,\beta) &= \left(\frac{2\beta}{\sqrt{\pi}}\right) e^{-(\brgt^2+\brlt^2)} \\
  \tDerfmS(\rlt,\rgt;\beta) &= G_H(\rlt,\rgt,\beta)\rlt \\
  \tDerfmF(\rlt,\rgt;\beta) &= G_H(\rlt,\rgt,\beta)\rlt\left(1 + \frac{2\brlt^2(\brgt^2+1)}{3} \right)
\end{split}
\end{equation}
have been defined where the max and min function are now referenced implicitly through $\{\rgt,\rlt\}$
and the subscript $H$ on the Gausian function denotes the half-space normalization. The functions
$\tDerfmS(\rlt,\rgt;\beta)$ and $\tDerfmF(\rlt,\rgt;\beta)$ are 2nd and 4th order small $\rlt$ approximations
to $\Derfm(\rlt,\rgt;\beta)$ respectively. The above expressions are problematic to evaluate numerically
for large $2\brgt\brlt$ as written. To
treat these cases, $\exp(2\brgt\brlt)$ is factored out of the $\sinh(2\brgt\brlt)$ and $\cosh(2\brgt\brlt)$ terms
and combined with the Gaussian, an exact rewriting stable at large $2\brgt\brlt$. Similarly, at small $2\brgt\brlt$, it is
convenient to insert the small argument expression of $\cosh()$ and $\sinhc()$ and simplify while at small $\brlt$ the small argument expansion of $\Derfm(\rlt,\rgt;\beta)$ is inserted followed by simplification. In this regard,
a useful Taylor series expansion is
\begin{eqnarray}
\frac{\erf(x+y)-\erf(x-y)}{2y} &=& \frac{2e^{-x^2}}{\sqrt{\pi}} \left [
  1 +  \frac{(2 x^2 - 1) y^2}{3} + \frac{(4 x^4 - 12 x^2 + 3) y^4}{30}
    + \frac{(8 x^6 - 60 x^4 + 90 x^2 - 15) y^6}{630} \right . \nonumber \\
& & \left .  \hspace*{0.75in}
  + \frac{(16x^8 - 224x^6 + 840x^4 - 840x^2 + 105) y^8}{22680} 
  \right] + {\cal O}(y^{10}) 
\end{eqnarray}
\\
It is straightfoward to apply the partial wave expansion given above to compute the regularized / screened
Hartree self-interaction of an atomic density,$\rho^{(\mathrm{atom})}(\br)$, on a domain of radius $R_{pc}$ centered
at the origin,
\begin{eqnarray}
E_H^{(\mathrm{self,screened})} &=&  \frac{e^2}{2} \int_{D(R_{pc})} \rd \br \int_{D(R_{pc})}\rd \br' 
      \rho^{(\mathrm{atom})}(\br)  \frac{\erf(\beta|\br-\br'|)}{|\br-\br'|} \rho^{(\mathrm{atom})}(\br') \\
 &=& \frac{e^2}{2} \sum_{l=0}^\infty\sum_{m=-l}^{l}\left(\frac{4\pi}{2l+1}\right) 
     \int_0^{R_{pc}} \rd r\int_0^{R_{pc}} \rd r' \left \{ \left ( r^2  
     \int \rd \Omega \rho^{(\mathrm{atom})}(r,\Omega)Y_{lm}(\Omega) \right)\PWerf_l(r,r';\beta) \right .\nonumber \\
 & & \hspace*{2.3in}\left . \times  \left( r'^2 \int \rd \Omega' \rho^{(\mathrm{atom})}(r',\Omega')Y_{lm}(\Omega')
                 \right ) \right \}\nonumber \\
 &=& \frac{e^2}{2}  \sum_{l=0}^\infty\sum_{m=-l}^{l}\left(\frac{4\pi}{2l+1}\right) 
     \int_0^{R_{pc}}\rd r \int_0^{R_{pc}}  \rd r'\left \{
           r^2  \rho^{(\mathrm{atom})}_{lm}(r)\PWerf_l(r,r';\beta)r'^2 \rho^{(\mathrm{atom})}_{lm}(r')\right \} 
\end{eqnarray}
where
\begin{equation}
\rho^{(\mathrm{atom})}_{lm}(r) = \int \rd \Omega \rho^{(\mathrm{atom})}(r,\Omega)Y_{lm}(\Omega)\ \ \ .
\end{equation}
Since the number of $l$ with non-zero $\rho^{(\mathrm{atom})}_{lm}(r)$ is finite ($l\leq l_{max}$)
(assuming a hydrogenic filling), the partial wave form has much lower computational complexity when
evaluated by numerical integration, $(l_{max}+1)^2 (n_{\Omega}n_r + n_r^2)$ than the original, $n_{\Omega}^2n_r^2$.
That is, in the partial wave expansion, 
the $\Omega$-integrals are seperable although $l_{max}^2$ of them must be performed and only the $r'r$ integration
is non-seperable - coupled through the partial wave coefficients, $\PWerf_l(r,r';\beta)$. More concretely, taking
$n_r\sim n^{1/3}$, $n_{\Omega}\sim n^{2/3}$ such that $n_rn_{\Omega}\sim n$, then
the partial wave method scales like $n$ and the brute force method like $n^2$.
Since atom size and hydrogenic filling are independent of system size, $n \sim {\cal O}(N^0)$.
In addition, the partial wave
expansion requires fewer angular integration points as the expression is truncated at $l_{max}$ while in the full
form, many angular integration points are used to essentially compute the neglible (zero) contribution
from high angular momentum components accurately.\\

For $l>3$, the partial waves can be generated by numerical integration using recursion relations, small value expansions and asymptotic series to generate the $i_l$ for arbitrary $l$ as follows. Introducing a quadrature the expression
for the $l^{\mathrm{th}}$ partial wave becomes
\begin{eqnarray}
\PWerf_l(r,r';\beta) &=& \left(\frac{2(2l+1)}{\sqrt{\pi}}\right)
\int_0^{\beta}\rd x\ \re^{-x^2\left(r^2 + r'^2\right)}i_{l}(2rr'x^2) \nonumber\\
&\approx& \left(\frac{2(2l+1)}{\sqrt{\pi}}\right)
\sum_i w_i\re^{-x_i^2\left(r^2 + r'^2\right)}i_{l}(2rr'x^2_i) \nonumber
\end{eqnarray}
which is fairly straightforward to implement.  In order to generate the $i_l(y)$ numerically,
the recursion relation,
\begin{equation}
i_{l+1}(y) = i_{l-1}(y) - \frac{(2l+1)i_{l}(y)}{y}
\end{equation}
can be employed, which is completely general. The recusion relation however, becomes numerically unstable at both small
and large $y$. At small $y$, the ascending series expansion given in (10.2.5) of Abramowitz and Stegun
\begin{equation}
  i_{l}(y) = \frac{y^l}{1\cdot 3 \cdot 5 \cdot (2l+1)}
             \left [ 1 + \frac{y^2}{1!(2l+3)2} + \frac{y^4}{2!(2l+3)(2l+5)4} + \ldots\right ]
\end{equation}  
can be applied while at large $y$ the asymptotic series expanion of DMLF formulae 10.40.1 and 10.17.1
\begin{equation}
i_{l}(y) = \frac{e^y}{2}\sum_{k=0}^\infty (-1)^k\frac{a_k\left(l+\frac{1}{2}\right)}{y^k}
\end{equation}
where
\begin{eqnarray}
  a_0(\nu) &=& 1 \nonumber \\
  a_k(\nu) &=& \frac{(4v^2-1^2)(4v^2-3^2) \ldots (4v^2-(2k-1)^2)}{k!8^k} 
\end{eqnarray}
is employed.
