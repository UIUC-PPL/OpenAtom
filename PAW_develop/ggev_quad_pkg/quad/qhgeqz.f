      SUBROUTINE QHGEQZ( JOB, COMPQ, COMPZ, N, ILO, IHI, H, LDH, T, LDT,
...
*
*  H       (input/output) REAL*16 array, dimension (LDH, N)
...
*
*  T       (input/output) REAL*16 array, dimension (LDT, N)
...
*
*  ALPHAR  (output) REAL*16 array, dimension (N)
*          The real parts of each scalar alpha defining an eigenvalue
*          of GNEP.
*
*  ALPHAI  (output) REAL*16 array, dimension (N)
...
*
*  BETA    (output) REAL*16 array, dimension (N)
...
*
*  Q       (input/output) REAL*16 array, dimension (LDQ, N)
...
*
*  Z       (input/output) REAL*16 array, dimension (LDZ, N)
...
*
*  WORK    (workspace/output) REAL*16 array, dimension (MAX(1,LWORK))
...
*     .. Parameters ..
*    $                     SAFETY = 1.0E+0 )
      REAL*16   HALF, ZERO, ONE, SAFETY
      PARAMETER          ( HALF = 0.5Q+0, ZERO = 0.0Q+0, ONE = 1.0Q+0,
     $                   SAFETY = 1.0Q+2 )
*     ..
*     .. Local Scalars ..
     ...
     $                   JR, MAXIT
      REAL*16   A11, A12, A1I, A1R, A21, A22, A2I, A2R, AD11,
...
*     .. External Functions ..
      LOGICAL            LSAME
      REAL*16   QLAMCH, QLANHS, QLAPY2, QLAPY3
      EXTERNAL           LSAME, QLAMCH, QLANHS, QLAPY2, QLAPY3
*     ..
*     .. External Subroutines ..
      EXTERNAL           QLAG2, QLARFG, QLARTG, QLASET, QLASV2, QROT,
     $                   XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( ICOMPQ.EQ.3 )
     $   CALL QLASET( 'Full', N, N, ZERO, ONE, Q, LDQ )
      IF( ICOMPZ.EQ.3 )
     $   CALL QLASET( 'Full', N, N, ZERO, ONE, Z, LDZ )
*
*     Machine Constants
*
      IN = IHI + 1 - ILO
      SAFMIN = QLAMCH( 'S' )
      SAFMAX = ONE / SAFMIN
      ULP = QLAMCH( 'E' )*QLAMCH( 'B' )
      ANORM = QLANHS( 'F', IN, H( ILO, ILO ), LDH, WORK )
      BNORM = QLANHS( 'F', IN, T( ILO, ILO ), LDT, WORK )
...
*
               IF( ILAZRO .OR. ILAZR2 ) THEN
                  DO 40 JCH = J, ILAST - 1
                     TEMP = H( JCH, JCH )
                     CALL QLARTG( TEMP, H( JCH+1, JCH ), C, S,
     $                            H( JCH, JCH ) )
                     H( JCH+1, JCH ) = ZERO
                     CALL QROT( ILASTM-JCH, H( JCH, JCH+1 ), LDH,
     $                          H( JCH+1, JCH+1 ), LDH, C, S )
                     CALL QROT( ILASTM-JCH, T( JCH, JCH+1 ), LDT,
     $                          T( JCH+1, JCH+1 ), LDT, C, S )
                     IF( ILQ )
     $                  CALL QROT( N, Q( 1, JCH ), 1, Q( 1, JCH+1 ), 1,
     $                             C, S )
                     IF( ILAZR2 )
...
*
                  DO 50 JCH = J, ILAST - 1
                     TEMP = T( JCH, JCH+1 )
                     CALL QLARTG( TEMP, T( JCH+1, JCH+1 ), C, S,
     $                            T( JCH, JCH+1 ) )
                     T( JCH+1, JCH+1 ) = ZERO
                     IF( JCH.LT.ILASTM-1 )
     $                  CALL QROT( ILASTM-JCH-1, T( JCH, JCH+2 ), LDT,
     $                             T( JCH+1, JCH+2 ), LDT, C, S )
                     CALL QROT( ILASTM-JCH+2, H( JCH, JCH-1 ), LDH,
     $                          H( JCH+1, JCH-1 ), LDH, C, S )
                     IF( ILQ )
     $                  CALL QROT( N, Q( 1, JCH ), 1, Q( 1, JCH+1 ), 1,
     $                             C, S )
                     TEMP = H( JCH+1, JCH )
                     CALL QLARTG( TEMP, H( JCH+1, JCH-1 ), C, S,
     $                            H( JCH+1, JCH ) )
                     H( JCH+1, JCH-1 ) = ZERO
                     CALL QROT( JCH+1-IFRSTM, H( IFRSTM, JCH ), 1,
     $                          H( IFRSTM, JCH-1 ), 1, C, S )
                     CALL QROT( JCH-IFRSTM, T( IFRSTM, JCH ), 1,
     $                          T( IFRSTM, JCH-1 ), 1, C, S )
                     IF( ILZ )
     $                  CALL QROT( N, Z( 1, JCH ), 1, Z( 1, JCH-1 ), 1,
     $                             C, S )
   50             CONTINUE
                  GO TO 70
               END IF
...
*
   70    CONTINUE
         TEMP = H( ILAST, ILAST )
         CALL QLARTG( TEMP, H( ILAST, ILAST-1 ), C, S,
     $                H( ILAST, ILAST ) )
         H( ILAST, ILAST-1 ) = ZERO
         CALL QROT( ILAST-IFRSTM, H( IFRSTM, ILAST ), 1,
     $              H( IFRSTM, ILAST-1 ), 1, C, S )
         CALL QROT( ILAST-IFRSTM, T( IFRSTM, ILAST ), 1,
     $              T( IFRSTM, ILAST-1 ), 1, C, S )
         IF( ILZ )
     $      CALL QROT( N, Z( 1, ILAST ), 1, Z( 1, ILAST-1 ), 1, C, S )
*
...
*
            CALL QLAG2( H( ILAST-1, ILAST-1 ), LDH,
...
*
         ...
         CALL QLARTG( TEMP, TEMP2, C, S, TEMPR )
*
*        Sweep
*
         DO 190 J = ISTART, ILAST - 1
            IF( J.GT.ISTART ) THEN
               TEMP = H( J, J-1 )
               CALL QLARTG( TEMP, H( J+1, J-1 ), C, S, H( J, J-1 ) )
               H( J+1, J-1 ) = ZERO
            END IF
*
            DO 140 JC = J, ILASTM
...
*
            TEMP = T( J+1, J+1 )
            CALL QLARTG( TEMP, T( J+1, J ), C, S, T( J+1, J+1 ) )
            T( J+1, J ) = ZERO
*
...
*                       (  0  B22 )
*
            CALL QLASV2( T( ILAST-1, ILAST-1 ), T( ILAST-1, ILAST ),
...
*
            ...
            END IF
*
            CALL QROT( ILASTM+1-IFIRST, H( ILAST-1, ILAST-1 ), LDH,
     $                 H( ILAST, ILAST-1 ), LDH, CL, SL )
            CALL QROT( ILAST+1-IFRSTM, H( IFRSTM, ILAST-1 ), 1,
     $                 H( IFRSTM, ILAST ), 1, CR, SR )
*
            IF( ILAST.LT.ILASTM )
     $         CALL QROT( ILASTM-ILAST, T( ILAST-1, ILAST+1 ), LDT,
     $                    T( ILAST, ILAST+1 ), LDT, CL, SL )
            IF( IFRSTM.LT.ILAST-1 )
     $         CALL QROT( IFIRST-IFRSTM, T( IFRSTM, ILAST-1 ), 1,
     $                    T( IFRSTM, ILAST ), 1, CR, SR )
*
            IF( ILQ )
     $         CALL QROT( N, Q( 1, ILAST-1 ), 1, Q( 1, ILAST ), 1, CL,
     $                    SL )
            IF( ILZ )
     $         CALL QROT( N, Z( 1, ILAST-1 ), 1, Z( 1, ILAST ), 1, CR,
     $                    SR )
*
...
*
               IF( J.GT.ISTART ) THEN
                  ...
                  V( 3 ) = H( J+2, J-1 )
*
                  CALL QLARFG( 3, H( J, J-1 ), V( 2 ), 1, TAU )
                  V( 1 ) = ONE
                  H( J+1, J-1 ) = ZERO
                  H( J+2, J-1 ) = ZERO
               END IF
