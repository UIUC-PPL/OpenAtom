      SUBROUTINE QTGEVC( SIDE, HOWMNY, SELECT, N, S, LDS, P, LDP, VL,
...
*
*  S       (input) REAL*16 array, dimension (LDS,N)
...
*
*  P       (input) REAL*16 array, dimension (LDP,N)
...
*
*  VL      (input/output) REAL*16 array, dimension (LDVL,MM)
...
*          SIDE = 'L' or 'B', LDVL >= N.
*
*  VR      (input/output) REAL*16 array, dimension (LDVR,MM)
...
*          columns.
*
*  WORK    (workspace) REAL*16 array, dimension (6*N)
*
*  INFO    (output) INTEGER
*          = 0:  successful exit.
...
*     .. Local Scalars ..
     ...
     $                   J, JA, JC, JE, JR, JW, NA, NW
      REAL*16   ACOEF, ACOEFA, ANORM, ASCALE, BCOEFA, BCOEFI,
...
*     ..
*     .. Local Arrays ..
      REAL*16   BDIAG( 2 ), SUM( 2, 2 ), SUMS( 2, 2 ),
     $                   SUMP( 2, 2 )
*     ..
*     .. External Functions ..
      LOGICAL            LSAME
      REAL*16   QLAMCH
      EXTERNAL           LSAME, QLAMCH
*     ..
*     .. External Subroutines ..
      EXTERNAL           QGEMV, QLABAD, QLACPY, QLAG2, QLALN2, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      BIG = ONE / SAFMIN
      CALL QLABAD( SAFMIN, BIG )
      ULP = QLAMCH( 'Epsilon' )*QLAMCH( 'Base' )
      SMALL = SAFMIN*N / ULP
      BIG = ONE / SMALL
      BIGNUM = ONE / ( SAFMIN*N )
*
...
*              Complex eigenvalue
*
               CALL QLAG2( S( JE, JE ), LDS, P( JE, JE ), LDP,
...
*
               CALL QLALN2( .TRUE., NA, NW, DMIN, ACOEF, S( J, J ), LDS,
...
*
            ...
            IF( ILBACK ) THEN
               DO 170 JW = 0, NW - 1
                  CALL QGEMV( 'N', N, N+1-JE, ONE, VL( 1, JE ), LDVL,
     $                        WORK( ( JW+2 )*N+JE ), 1, ZERO,
     $                        WORK( ( JW+4 )*N+1 ), 1 )
  170          CONTINUE
               CALL QLACPY( ' ', N, NW, WORK( 4*N+1 ), N, VL( 1, JE ),
     $                      LDVL )
               IBEG = 1
            ELSE
               CALL QLACPY( ' ', N, NW, WORK( 2*N+1 ), N, VL( 1, IEIG ),
     $                      LDVL )
               IBEG = JE
            END IF
*
...
*              Complex eigenvalue
*
               CALL QLAG2( S( JE-1, JE-1 ), LDS, P( JE-1, JE-1 ), LDP,
     $                     SAFMIN*SAFETY, ACOEF, TEMP, BCOEFR, TEMP2,
     $                     BCOEFI )
               IF( BCOEFI.EQ.ZERO ) THEN
                  INFO = JE - 1
...
*
               CALL QLALN2( .FALSE., NA, NW, DMIN, ACOEF, S( J, J ),
