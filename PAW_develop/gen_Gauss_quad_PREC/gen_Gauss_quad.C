...
// Set up orders 1 ...n
	for (int k=1; k<n+1; k++) {
		...
		for (int j=0; j<k; j++) {
	      ...
	      //++++++++++++++++++++++++++++++++++++++++
      	      // compute the moments O of the poly over w(x) starting at j=k (O is upper triable) O== int x^j w(x) poly_k(x)
		for (int j=k; j<n+1; j++) {
	...
	// local pointer to the nth polynomialwhose roots we want
	double *a = poly[n].a;
	double *c = poly[n].c;
...
//==========================================================================
//  Copy out, sort the nodes and make sure they are good zeros

	for (int i=0; i<n; i++) {node[i] = ALPHAR[i];}
	sort(node, node + n);

	testnodes(n, node, a,iopt);
...
void testnodes(int n, double * x, double * a, int iopt) {
	...
	for (int i=0; i<n; i++) {
		...
		if (fabs(zero) > 1.0e-5) {
		  ierr++;
		  if(iopt==1){PRINTF("  The %dth zero, %.10g, has eror %.10g\n", i,x[i],err);}
		}//endif
	}//end for
	if (ierr > 0) {
		PRINTF("  @@@@@@@@@@@@@@@@@@@@_error_@@@@@@@@@@@@@@@@@@@@\n");
	  	PRINTF("    Max err in zeros %.10g > tolerance\n", err);
...
//---------------------------------------------------------------------------
	...
	for (int k=0; k<n+1; k++) {
		...
		for (int kp=0; kp<iup; kp++) {
			...
			if (iopt == 1) {
				PRINTF("  Ortho test %d, %d) = %.10g with error %.10g\n", k, kp, result, err);
			} // end if
			if (fabs(err) > 1.0e-5&& iopt == 0) {
				ierr++;
